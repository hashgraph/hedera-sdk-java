...
public class Reference {
	...
	public Reference(byte[] data) {
		...
		// store in scrambled form so changing one bit of reference changes the entire
	...
	public Reference(String dataString) {
		if (dataString.matches("^<[a-zA-Z0-9]*>$")) { // base 62 encoding
			...
			int len128Bits = lenInBitSize(128, words.size());
			int len256Bits = lenInBitSize(256, words.size());
			int len384Bits = lenInBitSize(384, words.size());
		...
		byte[] dataUnscrambled = new byte[data.length - 1];
	...
	/**
	 * return a string representation of the given byte array in hex, with a each
	 * byte becoming two characters (including a leading zero, if necessary). Only
	 * use the array from index firstIndex to index lastIndex. A lastIndex of -1
	...
	static public int lenInBitSize(int bitSize, int lengthDenominator) {
		int numerator = bitSize + 8;
		double denominator = Math.log(lengthDenominator) / Math.log(2);
		int ans = (int) Math.ceil(numerator / denominator);
		return ans;
	}

	static public List<String> wordIndices(String dataString) {
		List<String> allWords = Collections.synchronizedList(new ArrayList<String>());
		Matcher m = Pattern.compile("[a-zA-Z]+").matcher(dataString);
		while (m.find()) {
	...
	static public int[] allWordsExtraction(List<String> allWords, List<String> words) {
