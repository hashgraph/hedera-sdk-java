...
public class ECDSAPrivateKeyTest {
    ...
    @Test
    @DisplayName("PEM decode is compatible with OpenSSL")
    void openSSLPEMCompatibility() throws IOException {
        // Checks the SDK's compatibility with keys generated by OpenSSL.
        // openssl ecparam -genkey -name secp256k1 -rand /dev/urandom -noout -out ecdsa_priv.pem
        // openssl ec -in ecdsa_priv.pem -pubout -outform DER -conv_form compressed | xxd -p -c 0
        var PRIVATE_KEY_PEM = "-----BEGIN EC PRIVATE KEY-----\n" +
            ...
            "-----END EC PRIVATE KEY-----";
        var PUBLIC_KEY_DER = "3036301006072a8648ce3d020106052b8104000a03220002a9fe4198c781ce453545af5401b649a37b72b553a56fb7ab4dcdfa2d12cfdb49";

        var message = "message".getBytes(StandardCharsets.UTF_8);
        var ecdsaPrivateKey = PrivateKey.fromPem(PRIVATE_KEY_PEM);
        var ecdsaPublicKey = PublicKey.fromStringDER(PUBLIC_KEY_DER);
        var ecdsaSignature = ecdsaPrivateKey.sign(message);

        assertThat(ecdsaPrivateKey.getPublicKey()).isEqualTo(ecdsaPublicKey);
        assertThat(ecdsaPublicKey.verify(message, ecdsaSignature)).isTrue();
        assertThat(ecdsaPrivateKey.getPublicKey().toStringDER()).isEqualTo(PUBLIC_KEY_DER);
    }

    @Test
    @DisplayName("PEM decode encrypted is compatible with OpenSSL")
    void openSSLPEMEncryptedCompatibility() throws IOException {
        // Checks the SDK's compatibility with keys generated by OpenSSL.
        // openssl ecparam -genkey -name secp256k1 | openssl ec -aes-128-cbc -out ecdsa_enc.pem
        // openssl ec -in ecdsa_enc.pem -pubout -outform DER -conv_form compressed | xxd -p -c 0
        var PRIVATE_KEY_PEM = "-----BEGIN EC PRIVATE KEY-----\n" +
            ...
            "-----END EC PRIVATE KEY-----";
        var PUBLIC_KEY_DER = "3036301006072a8648ce3d020106052b8104000a032200025f0d14a7562d6319e5b8f91620d2ce9ad13d9abf21cfe9bd0a092c0f35bf1701";

        var message = "message".getBytes(StandardCharsets.UTF_8);
        var ecdsaPrivateKey = PrivateKey.fromPem(PRIVATE_KEY_PEM, "asdasd123");
        var ecdsaPublicKey = PublicKey.fromStringDER(PUBLIC_KEY_DER);
        var ecdsaSignature = ecdsaPrivateKey.sign(message);
    ...
    @Test
    @DisplayName("DER encode/decode is compatible with OpenSSL")
    void openSSLDERCompatibility() {
        // Checks the SDK's compatibility with keys generated by OpenSSL.
        // Steps:
        // 1. Generate the private and public PEM files
        // openssl ecparam -genkey -name secp256k1 -rand /dev/urandom -noout -out ecdsa_priv.pem
        // openssl ec -in ecdsa_priv.pem -pubout -out ecdsa_pub.pem
        // 2. Print out the DER keys
        // openssl ec -in ecdsa_priv.pem -outform DER -conv_form compressed | xxd -p -c 0
        // openssl ec -in ecdsa_pub.pem -pubin -outform DER -conv_form compressed | xxd -p -c 0
        var PRIVATE_KEY_DER = "30540201010420ac318ea8ff8d991ab2f16172b4738e74dc35a56681199cfb1c0cb2e7cb560ffda00706052b8104000aa124032200036843f5cb338bbb4cdb21b0da4ea739d910951d6e8a5f703d313efe31afe788f4";
        var PUBLIC_KEY_DER = "3036301006072a8648ce3d020106052b8104000a032200036843f5cb338bbb4cdb21b0da4ea739d910951d6e8a5f703d313efe31afe788f4";

        var message = "message".getBytes(StandardCharsets.UTF_8);
        var ecdsaPrivateKey = PrivateKey.fromStringDER(PRIVATE_KEY_DER);
        var ecdsaPublicKey = PublicKey.fromStringDER(PUBLIC_KEY_DER);
        var ecdsaSignature = ecdsaPrivateKey.sign(message);

        assertThat(ecdsaPrivateKey.getPublicKey()).isEqualTo(ecdsaPublicKey);
        assertThat(ecdsaPublicKey.verify(message, ecdsaSignature)).isTrue();
        assertThat(ecdsaPrivateKey.toStringDER()).isEqualTo(PRIVATE_KEY_DER);
        assertThat(ecdsaPrivateKey.getPublicKey().toStringDER()).isEqualTo(PUBLIC_KEY_DER);
    }
}
